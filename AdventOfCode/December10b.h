...
namespace December10 {
    ...
    class ComplexKnotHash
        : protected SimpleKnotHash
    {
    public:
        ComplexKnotHash() {};
        ComplexKnotHash(const std::string& fileName);
        ~ComplexKnotHash() = default;
    ...
    public:
        static void ConvertStringToAsciiVector(const std::string& input, std::vector<int>& output);
        static bool ReduceToDenseHash(const std::vector<int>& sparseHash, std::vector<int>& denseHash);
        static void ConvertVectorToHexValue(const std::vector<int>& hash, std::string& hexString);
        std::string GetKnotHashHexValue() const;

    private:
        const std::vector<int> m_bonusSequenceOfLengths = { 17, 31, 73, 47, 23 };
    };
